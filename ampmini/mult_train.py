...
# from torch.utils.tensorboard import SummaryWriter
from datetime import datetime
from network import SequenceTransformer, FocalLoss, TranCNNnew
from AMP_dataset import AMP_Dataset

# Setup logging
log_dir = "logs/out/AMP_s2/dataparallel"
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, "log.txt")
logging.basicConfig(
    level=logging.INFO,
...
def train(path="", batch_size=64, learning_rate=0.001, num_epochs=102, save_model_dir="n_models/out/"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logging.info(f"Training on {device} using DataParallel")
    ...
    for epoch in range(num_epochs):
        ...
        data_time = time.time()  # Start time for the entire epoch
        for i, batch in enumerate(train_loader):
            dataload_start = time.time()
            dt = dataload_start - data_time
            total_dataload_time += dt
            # print("data_time",dt)
        ...
        os.makedirs(save_model_dir, exist_ok=True)
        if epoch % 10 == 0:
            model_path = os.path.join(save_model_dir, f'newTCNN{epoch}.pth.tar')
            torch.save({'state_dict': model.module.state_dict() if isinstance(model, DataParallel) else model.state_dict()}, model_path)
            logging.info(f"Model saved at {model_path}")
...
if __name__ == "__main__":
    train(path="./data/new_AMP_sequences.csv")
